...
import com.friday.assistant.voice.FridayWakeCoordinator
...
/** Native diagnostic HUD with a single-owner microphone path for direct speech tests. */
class FridayHudActivity : ComponentActivity() {
    ...
    private var directListening = false
    ...
    private fun observeRuntime(){
        scope.launch { FridayStateFlow.state.collect { s ->
            ...
            val healthy=s.healthy
            status.setTextColor(if(healthy) cyan else red)
            if (!directListening) orb.setTextColor(if(healthy) cyan else red)
            val a=(s.audioAmplitude.coerceIn(0f,1f)*100).toInt()
            val blocks=(a/10).coerceIn(0,10)
            if (!directListening) micMeter.text="MIC INPUT  ${"█".repeat(blocks)}${"░".repeat(10-blocks)}  $a%"
            if(!directListening && a>8) orb.text="◉" else if(!directListening && (s.stage.contains("LISTEN",true)||s.stage.contains("READY",true))) orb.text="○"
        } }
    }
    ...
    private fun connectGemini(){
        ...
        keyInput.setText("")
        setStatus("GEMINI CONNECTED","API key stored securely",true)
    ...
    private fun startDirectListening(){
        ...
        if (directListening) return
        directListening = true
        listenButton.isEnabled=false
        voice?.destroy()
        voice=null
        setStatus("MIC RELEASE", "Releasing wake microphone before direct speech test", true)
        FridayWakeCoordinator.pauseForSpeech {
            if (isFinishing || isDestroyed) return@pauseForSpeech
            if (checkSelfPermission(Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED) {
                directListening=false
                listenButton.isEnabled=false
                setStatus("MIC PERMISSION","Microphone permission was revoked",false)
                return@pauseForSpeech
            }
            setStatus("LISTENING","Microphone is ready — speak now",true)
            val manager=VoiceManager(this,object:VoiceManager.Listener{
                ...
                override fun onResult(text:String){
                    ...
                    if(text.isBlank()){setStatus("NO SPEECH","Nothing was recognized. Try again.",false);FridayWakeCoordinator.resumeAfterSpeech();return}
                    setStatus("HEARD",text,true)
                    ensureAgent().handle(text){answer,_->setStatus("SPEAKING",answer,true);tts.speak(answer){setStatus("READY","Command cycle complete • press LISTEN NOW again",true);FridayWakeCoordinator.resumeAfterSpeech()}}
                }
                override fun onError(message:String){directListening=false;listenButton.isEnabled=true;setStatus("VOICE ERROR",message,false);FridayWakeCoordinator.resumeAfterSpeech()}
            })
            voice=manager
            manager.start()
        }
    ...
    override fun onDestroy(){if(directListening) FridayWakeCoordinator.resumeAfterSpeech();scope.cancel();voice?.destroy();voice=null;agent?.close();agent=null;tts.shutdown();super.onDestroy()}
}
