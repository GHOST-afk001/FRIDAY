...
/**
 * FRIDAY's model-facing brain. Deterministic Android actions are handled locally without
 * network latency; every open-ended conversation request is sent to Gemini 3.6 Flash when
 * the encrypted runtime key is configured. There is no silent static emotional fallback.
 */
class FridayAgent(context: Context) {
    ...
    fun handle(input: String, callback: (String, Boolean) -> Unit) {
        if (closed) return
        FridayRuntime.update("UNDERSTANDING", "Interpreting Imroz Sir's request", true)
        ...
        if (!gemini.isConfigured()) {
            val answer = "Imroz Sir, Gemini brain abhi configured nahi hai. AI BRAIN setup mein apni Gemini API key add kijiye; uske baad main open-ended requests directly Gemini 3.6 Flash ko bhejungi."
            FridayRuntime.update("BRAIN NOT CONFIGURED", "Gemini API key is required for FRIDAY intelligence", false)
        ...
        activeRequest = brainScope.launch {
            try {
                ...
                FridayRuntime.update("AI THINKING", "Gemini 3.6 Flash is reasoning over context and intent", true)
                ...
                withContext(Dispatchers.Main.immediate) {
                    if (!closed && requestGeneration.get() == myGeneration) {
                        FridayRuntime.update("RESPONSE READY", "Gemini response verified and ready for speech", true)
    ...
    private fun handleIdentity(input: String): String? {
        ...
        val match = Regex("^(?:my name is|mera naam|mera name|call me)\\s+([\\p{L}][\\p{L} .'-]{1,29})(?:\\s+hai)?[.!]?$", RegexOption.IGNORE_CASE).find(value)
            ?: return null
