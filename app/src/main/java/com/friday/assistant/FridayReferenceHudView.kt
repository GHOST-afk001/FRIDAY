...
class FridayReferenceHudView(context: Context) : View(context) {
    ...
    private val frameTicker = object : Runnable { override fun run() { if (!isAttachedToWindow) return; runCatching { sample(); frame++; invalidate() }; handler.postDelayed(this, 250L) } }
    ...
    override fun onAttachedToWindow() { super.onAttachedToWindow(); handler.removeCallbacks(frameTicker); runCatching { handler.post(frameTicker) } }
    override fun onDetachedFromWindow() { handler.removeCallbacks(frameTicker); super.onDetachedFromWindow() }
    fun render(value: FridayUiState) { state = value; runCatching { invalidate() } }
    ...
    override fun onDraw(canvas: Canvas) {
        try {
            ...
            paint.color = orange
            paint.textSize = 20f
            canvas.drawText("FRIDAY  •  SAFE MODE", 32f, 56f, paint)
        }
    }
