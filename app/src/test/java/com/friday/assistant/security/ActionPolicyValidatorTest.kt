...
class ActionPolicyValidatorTest {
    ...
    @Test
    fun overDeepSequenceIsRejectedInsteadOfPartiallyValidated() {
        var nested: FridayAction = FridayAction.YouTube
        repeat(6) {
            nested = FridayAction.Sequence(listOf(nested))
        }

        val result = policy.validate(nested)
        val rejected = assertIs<ActionPolicyValidator.Outcome.Rejected>(result)
        assertEquals("The action plan is nested too deeply.", rejected.reason)
    }
}
